class scene3 extends Phaser.Scene {
    ...
    preload() {
        ...
        document.getElementById('description').innerHTML = '<h2>Level 3 - Confrontation</h2>';
    }
    ...
    create() {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            let yPosition = this.enemyY;
            let spriteKey = "platformer";
            let frame = "character_roundRed.png";

            if (i === 3) {
                frame = "character_roundYellow.png";
                yPosition += 50;
            }

            let enemy = this.physics.add.sprite(xPosition, yPosition, spriteKey, frame).setOrigin(0.5, 0.5);
            enemy.speed = this.enemySpeeds[i % this.enemySpeeds.length];
            if (i === 3) {
                enemy.setScale(3.0);
                enemy.speed *= 1.5;
            }
    ...
    update(time, delta) {
        ...
        this.my.sprite.enemies.forEach(enemy => {
            ...
            enemy.y += Math.sin(time / 1000) * enemy.speed * delta / 1000;

            if (enemy.x < 50 || enemy.x > this.sys.game.config.width - 50) {
                enemy.speed *= -1;
            }
            enemy.x = Phaser.Math.Clamp(enemy.x, 50, this.sys.game.config.width - 50);
            enemy.y = Phaser.Math.Clamp(enemy.y, 50, this.sys.game.config.height - 50);
        });
