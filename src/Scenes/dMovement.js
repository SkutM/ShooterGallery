class dMovement extends Phaser.Scene {
    ...
    preload() {
        ...
        document.getElementById('description').innerHTML = '<h2>Level 1 - Tutorial, Space = Shoot, A = Left, D = Right</h2>'
    }
    ...
    create() {
        ...
        for (let i = 0; i < 2; i++) {
            let enemy = this.physics.add.sprite(this.bodyX + i * 100, this.enemyY, "platformer", "character_roundRed.png").setOrigin(0.5, 0.5);
    ...
    update(time, delta) {
        ...
        this.my.sprite.enemies.forEach(enemy => {
            if (enemy.active) {
                enemy.x += this.enemySpeed * this.enemyDirection * delta / 1000;
                if (enemy.x >= this.sys.game.config.width - enemy.width / 2 || enemy.x <= enemy.width / 2) {
                    this.enemyDirection *= -1;
                }
            }
        });
